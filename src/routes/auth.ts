...
// Register handler com validacao via middleware
const registerHandler = catchAsync(async (req: Request, res: Response) => {
  const { name, email, password, role, churchId } = req.body;

  console.log('Processando registro:', { name, email, password: '***', role, churchId, hasFile: !!(req as any).file });
  ...
  // Handle avatar upload se houver
  let avatarUrl = null;
  if ((req as any).file) {
    avatarUrl = `/uploads/avatars/${(req as any).file.filename}`;
...
// Rotas de registro com validação via middleware
const registerJSON = [validate(registerValidation), registerHandler];
const registerWithAvatar = [uploadAvatar.single('avatar'), validate(registerValidation), registerHandler];
...
// Handler para registro após validação
